...
Start a Zabbix proxy container as follows:
    ...
    docker run --name some-zabbix-proxy-pgsql -e DB_SERVER_HOST="some-postgres-server" -e POSTGRES_USER="some-user" -e POSTGRES_PASSWORD="some-password" -e ZBX_HOSTNAME=some-hostname -e ZBX_SERVER_HOST=some-zabbix-server -d zabbix/zabbix-proxy-pgsql:tag

Where `some-zabbix-proxy-pgsql` is the name you want to assign to your container, `some-pgsql-server` is IP or DNS name of PostgreSQL server, `some-user` is user to connect to Zabbix database on PostgreSQL server, `some-password` is the password to connect to PostgreSQL server, `some-hostname` is the hostname, it is Hostname parameter in Zabbix proxy configuration file, `some-zabbix-server` is IP or DNS name of Zabbix server and `tag` is the tag specifying the version you want. See the list above for relevant tags, or look at the [full list of tags](https://hub.docker.com/r/zabbix/zabbix-proxy-pgsql/tags/).
...
```console
$ docker run --name some-zabbix-server --link some-zabbix-proxy-pgsql:zabbix-proxy-pgsql -d zabbix/zabbix-server:latest
```
...
```console
$ docker run --name some-zabbix-proxy-pgsql --link some-zabbix-server:zabbix-server -d zabbix/zabbix-proxy-pgsql:latest
```
