...
check_variables_mysql() {
    ...
    if [ ! -n "${MYSQL_USER}" ] && [ "${MYSQL_RANDOM_ROOT_PASSWORD,,}" == "true" ]; then
        echo "**** Impossible to use Oracle server because of unknown Zabbix user and random 'root' password"
        exit 1
    fi

    if [ ! -n "${MYSQL_USER}" ] && [ ! -n "${MYSQL_ROOT_PASSWORD}" ] && [ "${MYSQL_ALLOW_EMPTY_PASSWORD,,}" != "true" ]; then
        echo "*** Impossible to use Oracle server because 'root' password is not defined and it is not empty"
        exit 1
    fi
    ...
    DB_SERVER_DBNAME=${ORACLE_DATABASE_NAME:-"zabbix"}
    DB_SERVER_SERVICE_NAME=${ORACLE_SERVICE:-"zabbix"}
}
...
check_db_connect_mysql() {
    ...
    while [ ! "$(echo "select 1 from dual;" | sqlcl -S "${MYSQL_USER}/${MYSQL_PASSWORD}@${ORACLE_DB_CONNECTION}")" ]; do
...
oracle_query() {
    ...
    result=$(echo "$query;" | sqlcl -S "${MYSQL_USER}/${MYSQL_PASSWORD}@${ORACLE_DB_CONNECTION}")
    unset MYSQL_PWD
...
exec_sql_file() {
    ...
    $command "$sql_script" | sqlcl -S "${MYSQL_USER}/${MYSQL_PASSWORD}@${ORACLE_DB_CONNECTION}" 1>/dev/null

    unset MYSQL_PWD
}

create_db_user_mysql() {
    # echo "*********** ${CREATE_ZBX_DB_USER}"
    # [ "${CREATE_ZBX_DB_USER}" == "true" ] || return

    echo "** Creating '${DB_SERVER_ZBX_USER}' user in Oracle database"

    USER_EXISTS=$(oracle_query "SELECT 1 FROM all_users WHERE username = '${DB_SERVER_ZBX_USER}'")

    if [ -z "$USER_EXISTS" ]; then
        oracle_query "CREATE USER '${DB_SERVER_ZBX_USER}' IDENTIFIED BY \"${DB_SERVER_ZBX_PASS}\"" 1>/dev/null
    else
        oracle_query "ALTER USER ${DB_SERVER_ZBX_USER} IDENTIFIED BY \"${DB_SERVER_ZBX_PASS}\"" 1>/dev/null
    fi

    # oracle_query "GRANT ALL PRIVILEGES ON $DB_SERVER_DBNAME. * TO '${DB_SERVER_ZBX_USER}'@'%'" 1>/dev/null
}

create_db_database_mysql() {
    DB_EXISTS=$(oracle_query "SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='${DB_SERVER_DBNAME}'")

    if [ -z ${DB_EXISTS} ]; then
        echo "** Database '${DB_SERVER_DBNAME}' does not exist. Creating..."
        oracle_query "CREATE DATABASE ${DB_SERVER_DBNAME} CHARACTER SET ${DB_CHARACTER_SET} COLLATE ${DB_CHARACTER_COLLATE}" 1>/dev/null
        # better solution?
        oracle_query "GRANT ALL PRIVILEGES ON $DB_SERVER_DBNAME. * TO '${DB_SERVER_ZBX_USER}'@'%'" 1>/dev/null
    else
...
create_db_schema_mysql() {
    DBVERSION_TABLE_EXISTS=$(oracle_query "select table_name from user_tables where table_name = 'USERS'")
    if [[ "$DBVERSION_TABLE_EXISTS" =~ "USERS" ]]; then
        echo "** Table '${DB_SERVER_DBNAME}.dbversion' already exists."
        ZBX_DB_VERSION=$(oracle_query "SELECT mandatory FROM ${DB_SERVER_DBNAME}.dbversion")
    fi
...
update_zbx_config() {
    ...
    update_config_var $ZBX_CONFIG "DBName" "${DB_SERVER_SERVICE_NAME}"
