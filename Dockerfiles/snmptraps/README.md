...
These are the only official Zabbix snmptraps Docker images. They are based on Alpine Linux v3.24, Ubuntu 24.04 (noble), CentOS Stream 10 and Oracle Linux 10 images. The available versions of Zabbix snmptraps are:
...
This variable represents the date and time format in the output `snmptraps.log` file. By default, value is `+%Y%m%d.%H%M%S`. Please refer to the `date` command manual for more details about date and time formats.
...
The volume also can be used for custom configuration file ``snmptrapd_custom.conf``. The configuration file can be used for SNMPv3 authentication details.
