...
ca_cn='Zabbix MySQL TLS test CA'
server_cn='mysql-server'
client_cn='zabbix'

repo_root="$(realpath "$(dirname "${BASH_SOURCE[0]}")/../..")"
out_dir="${repo_root}/env_vars"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/zabbix-mysql-tls.XXXXXX")"
trap 'rm -rf -- "${tmp_dir}"' EXIT

umask 077

# Generate a local certificate authority for the MySQL TLS example
...
# Generate the MySQL server private key and certificate signing request
...
# Sign the MySQL server certificate with the local certificate authority
...
printf 'MySQL TLS certificates have been generated...\n'
