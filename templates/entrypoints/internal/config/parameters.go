...
// Parameter describes one Zabbix configuration parameter update.
...
// SetParameters applies configuration parameter updates in order.
func SetParameters(configPath string, parameters ...Parameter) error {
	for _, parameter := range parameters {
		if err := SetParameter(configPath, parameter.Name, parameter.Value); err != nil {
			return err
		}
	}
...
func logParameterChange(configPath, param string, values []string, changed bool) {
	hasValue := len(values) == 1 && values[0] == ""
	for _, value := range values {
		if value != "" {
			hasValue = true
			break
