...
func configureFeatureSwitches(env bootstrap.Environment, homeDir, configPath string) error {
	...
	if strings.EqualFold(env["ZBX_ENABLEPERSISTENTBUFFER"], "true") {
		...
		values = append(values,
			bootstrap.ConfigValue{Name: "EnablePersistentBuffer", Value: "0"},
			bootstrap.ConfigValue{Name: "PersistentBufferFile"},
			bootstrap.ConfigValue{Name: "PersistentBufferPeriod"},
		)
	}

	if strings.EqualFold(env["ZBX_ENABLESTATUSPORT"], "true") {
